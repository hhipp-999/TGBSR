function [S,U,obj,obj2,convegence_value] = LasarMy(Z_ini,W,num_cluster,lambda1,lambda2,miu,rho,max_iter)
convegence_value = [];
S = Z_ini;
A = S;
Nsamp = size(S{1},1);
...
for iter = 1:max_iter
    ...
    if iter > 1
        ...
        LSv = diag(sum(sum_S))-sum_S;
        LSv = (LSv+LSv')*0.5;
        tmp = 0.5*miu*eye(Nsamp)-miu*A{nv+1}+C{nv+1};%%%%%%%%%%%%%%%%%%%
%         tmp = 0.5*miu*eye(Nsamp)-0.5*miu*A{nv+1}-0.5*miu*A{nv+1}'+0.5*C{nv+1}+0.5*C{nv+1}';
%         tmp = 0.5*miu*eye(Nsamp)-A{nv+1}+C{nv+1}/miu;
        tmp = (tmp+tmp')*0.5; %%%%%%%%%%%%%%%%%%%%%%
        LSv = LSv + tmp;
        try
            opts.tol = 1e-4; 
    ...
    % ---------------- A ----------------- %
    S_tensor = cat(3, S{:,:},U*U');%%%%%%%%%%%%%%%%
    C_tensor = cat(3, C{:,:});
    Sv = S_tensor(:);
    Cv = C_tensor(:);
    [Av, objV] = wshrinkObj(Sv + 1/miu*Cv,lambda1/miu,[Nsamp,Nsamp,nv+1],0,1);%%%%%%%%%%%
    A_tensor = reshape(Av, [Nsamp,Nsamp,nv+1]);
    for iv = 1:nv
        A{iv} = A_tensor(:,:,iv);
        % -------- C{iv} ------- %
        C{iv} = C{iv}+miu*(S{iv}-A{iv});
    end
    A{nv+1} = A_tensor(:,:,nv+1);  %%%%%%%%%%%%%%%%%%%%
    C{nv+1} = C{nv+1}+miu*(U*U'-A{nv+1}); %%%%%%%%%%%%%%%%
...
%     diff_U = max(abs(U(:)-U_pre(:)));
    ...
    leqm = cat(3, leq{:,:});
    leqm2 = max(abs(leqm(:)));
    convegence_value = [convegence_value;leqm2];
    clear leqm leq
